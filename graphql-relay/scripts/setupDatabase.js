...
import creators from '../src/schema/setup';
 ...
 * @returns {Function}
 */
const createDocuments = (creator) => async (state) => {
  const input = await creator(state);
...
const setupDatabase = async () => {
  ...
  for (const { name } of collections) {
    if (await mongoose.connection.db.collection(name).count()) {
      console.log('The database has already been initialized. Skipping.');
